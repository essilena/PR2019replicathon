...
Notice that the core tidyverse packages are listed under "`Attaching packages`" and loaded without having to load each one separately. How wonderful!
...
The ["pipe" symbol (`%>%`)](https://magrittr.tidyverse.org/reference/pipe.html) is a commonly used feature of the tidyverse. It was originally defined in the (cleverly named) [`magrittr`](https://magrittr.tidyverse.org/index.html) package, but is also included in the `dplyr` tidyverse package. The `%>%` symbol can seem confusing and intimidating at first. However, once you understand the basic idea, it can become addicting!
...
While the `dplyr` package includes many functions for manipulating data, we will have a hard time using these tools if our data is not in the correct form. For example, what if we want to compare the viability scores between two drugs in the CCLE study? To do this, we would want the two drugs to be in separate columns, so that we can compare them side-by-side. No amount of subsetting or mutating the table will get us there.
...
For a complete book on how to do data science using R and the tidyverse, we highly recommend the **R for Data Science** book, [available for free online](https://r4ds.had.co.nz/), by Garrett Grolemund and Hadley Wickham.

More practically, the [accompanying websites for the tidyverse packages](https://www.tidyverse.org/) are absolutely amazing, and a great resource for trying to understand how these functions work. When in doubt [ask](https://www.google.com/) [the](https://www.bing.com/) [internet](https://duckduckgo.com/).
