---
title: "Tutorial n: Exploring High Dimensional Data with PCA and Clustering"
output:
  html_document:
    toc: true
    toc_float: true
...
Principal Components Analysis is often used for dimensionality reduction when dealing with high-dimensional data. Here, we will look at a subset of the drug sensitivity data, from 
...
To start, let's load the data from the GDSC study and format it into `data.frame` of AUC
values.

```{r}
sumData <- readRDS('PR2019replicathon/data/summarizedPharmacoData.rds')
...
We are now ready to perform Principal Components Analysis (PCA), via the `prcomp` 
...
starting point would be to plot the top 2 principal components (PCs) and see what
