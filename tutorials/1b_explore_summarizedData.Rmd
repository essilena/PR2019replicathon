...
```{r read-rds}
cellLinesSummary <- readRDS(file.path("..", "data", "summarizedPharmacoData.rds"))
```
...
Notice that there are `r nrow(cellLinesSummary)` rows - each row here corresponds to a cell line-drug combination. 
...
```{r one-drug-subset}
azdSummary <- subset(cellLinesSummary, drug == "AZD0530")
```
