---
title: "Tutorial 1: Exploratory Analysis (raw data)"
output:
  html_document:
    toc: true
    toc_float: true
...
Also remember that [google](https://www.google.com) is one of the most important tool for 
data science. 
...
```{r quest4}
pharmacoData %>%
    dplyr::distinct(study, concentration) %>%
    dplyr::count(study)

pharmacoData %>%
    dplyr::group_by(study) %>%
    dplyr::summarize(n = n_distinct(concentration))

tapply(pharmacoData$concentration, pharmacoData$study, dplyr::n_distinct)
```
...
ggplot(pharmacoData, aes(x = log2(concentration))) +
...
```{r}
ggplot(pharmacoData, aes(x = viability, group = study, colour = study)) +
